...
def main():
    ...
    # Searching
    outputFile = outputFile + '<script type="text/javascript" language="JavaScript">function onSearch(){searchContent = document.getElementById("mySearch").value;var searchContentSC = transverter({type:"simplified",str:searchContent});var searchContentTC = transverter({type:"traditional",str:searchContent});var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){var searchText = storeId.rows[i].cells[0].innerHTML;if(searchText.match(searchContentTC) || searchText.match(searchContentSC) ||  searchText.toUpperCase().match(searchContent.toUpperCase())){storeId.rows[i].style.display=\'\';}else{storeId.rows[i].style.display=\'none\';}}}function clickFolder(){document.getElementById("mySearch").value = event.target.innerHTML;onSearch();}function frontpage(){document.getElementById("mySearch").value = "";var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){storeId.rows[i].style.display=\'\';}}</script>\n'
