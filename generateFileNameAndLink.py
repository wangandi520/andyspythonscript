...
def main():
    ...
    outputFile = outputFile + '<script type="text/javascript" language="JavaScript">function onSearch(){searchContent = document.getElementById(\'mySearch\').value.trim();var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){var searchText = storeId.rows[i].cells[0].innerHTML;if(searchText.match(searchContent) || searchText.toUpperCase().match(searchContent.toUpperCase())){storeId.rows[i].style.display=\'\';}else{storeId.rows[i].style.display=\'none\';}}}function doClickFolder(){if (document.getElementById("fileNameID").innerHTML){var searchValue = event.target.innerHTML;searchValue = searchValue.split("\\\\").slice(-1);document.getElementById("mySearch").value = searchValue;onSearch();}}function frontpage(){document.getElementById("mySearch").value = "";var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){storeId.rows[i].style.display=\'\';}}</script>\n'
