...
def main(inputPath):
    ...
    outputFile = outputFile + '，' + time.strftime("%Y%m%d", time.localtime()) + ')</a> ";document.getElementById(\"tips\").innerHTML = \"\";document.getElementById(\"mySearch\").focus();}start();var address = window.location.href.split(\"?search=\")[1];if (address){document.getElementById(\"mySearch\").value = decodeURI(address);onSearch();}start();</script>'
