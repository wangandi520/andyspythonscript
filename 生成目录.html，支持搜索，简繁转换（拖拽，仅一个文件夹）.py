...
def main(inputPath):
    ...
    # Searching
    outputFile = outputFile + '<script type="text/javascript" language="JavaScript">function onSearch(){searchContent = document.getElementById("mySearch").value;var searchContentSC = transverter({type:"simplified",str:searchContent}).trim();var searchContentTC = transverter({type:"traditional",str:searchContent}).trim();var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;var fileCount = 0;var fileSizeCount = 0;for(var i=1;i<rowsLength;i++){var searchText = storeId.rows[i].cells[0].innerHTML;var searchFileSize = storeId.rows[i].cells[1].innerHTML;if(searchText.match(searchContentTC) || searchText.match(searchContentSC) ||  searchText.toUpperCase().match(searchContent.toUpperCase())){storeId.rows[i].style.display=\'\';if (searchFileSize.substring(searchFileSize.length-2) == \'KB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2)) / 1024;}else if (searchFileSize.substring(searchFileSize.length-2) == \'MB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2));}else if (searchFileSize.substring(searchFileSize.length-2) == \'GB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2)) * 1024;}else if (searchFileSize.substring(searchFileSize.length-1) == \'B\'){;fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-1)) / 1024 / 1024;}fileCount =  fileCount + 1;}else{storeId.rows[i].style.display=\'none\';}}if (fileSizeCount > (1024 * 1024)){fileSizeCount = ((fileSizeCount / 1024 / 1024).toFixed(2)).toString() + \'TB\'}if (fileSizeCount > 1024){fileSizeCount = ((fileSizeCount / 1024).toFixed(2)).toString() + \'GB\'}if (fileSizeCount < 1024){fileSizeCount = fileSizeCount.toFixed(2).toString() + \'MB\'}document.getElementById("fileNameID").innerHTML = "<a href=\\"javascript:frontpage()\\">文件名</a> (搜索到\" + fileCount + \"项符合，共\" + fileSizeCount + \") \";}function doClickFolder(){if (document.getElementById("fileNameID").innerHTML){var searchValue = event.target.innerHTML;searchValue = searchValue.split("\\\\").slice(-1);document.getElementById("mySearch").value = searchValue;onSearch();}}function frontpage(){start();document.getElementById("mySearch").value = "";var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){storeId.rows[i].style.display=\'\';}}</script>\n'
